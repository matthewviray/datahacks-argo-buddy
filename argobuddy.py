...
def teacher():
    ...
    sal_prof = get_sal_profile() # Piped into Teacher view now!
    return (PAGE_TEACHER
...
# ═════════════════════════════════════════════════════════════════════════════
# STUDENT GAME PAGE (Charts removed!)
# ═════════════════════════════════════════════════════════════════════════════
PAGE = """<!DOCTYPE html>
<html lang="en">
<head>
...
.zone-nav{display:flex;justify-content:space-between;align-items:center;padding:.9rem 1.3rem;border-top:1px solid rgba(255,255,255,.08);background:rgba(0,0,0,.25);gap:.7rem;flex-wrap:wrap;margin-top:2rem;}
...
# ══════════════════════════════════════════════════════════════════════════════
# TEACHER DASHBOARD PAGE (4-Tab Curriculum Suite)
# ══════════════════════════════════════════════════════════════════════════════
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>ArgoBuddy — Curriculum Suite</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
.btn-back{padding:.38rem 1rem;border-radius:20px;background:rgba(255,255,255,.12);
          ...
          font-size:.78rem;font-weight:700;text-decoration:none;transition:background .2s;}
.btn-back:hover{background:rgba(255,255,255,.2)}

/* ── Tabs Navigation ── */
.tab-nav{
  display:flex;background:#2c3e50;padding:0 2rem;
  border-bottom:3px solid #00b4d8;box-shadow:0 2px 12px rgba(0,0,0,.15);
}
.tab-btn{
  background:none;border:none;color:rgba(255,255,255,.6);padding:1rem 1.5rem;
  font-weight:800;cursor:pointer;border-bottom:3px solid transparent;
  text-transform:uppercase;letter-spacing:.05em;font-size:.82rem;transition:all .2s;
}
.tab-btn:hover{color:rgba(255,255,255,.8);}
.tab-btn.active{color:white;border-bottom-color:#00d4ff;}

/* ── Tab Content Area ── */
.tab-pane{display:none;padding:1.5rem 2rem;height:calc(100vh - 125px);overflow-y:auto;}
.tab-pane.active{display:block;animation:fade .3s ease;}
@keyframes fade{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:translateY(0)}}

.module-grid{display:grid;grid-template-columns:1fr 340px;gap:1.5rem;height:100%;min-height:500px;}
@media(max-width:900px){.module-grid{grid-template-columns:1fr;height:auto;}}

/* ── Left Side (Evidence) ── */
.chart-box{
  background:white;border:1px solid #dde3ea;border-radius:12px;padding:1.5rem;
  display:flex;flex-direction:column;box-shadow:0 2px 8px rgba(0,0,0,.04);
}
.panel-title{
  font-size:.75rem;font-weight:800;text-transform:uppercase;letter-spacing:.1em;
  color:#8a99ab;margin-bottom:1rem;padding-bottom:.5rem;border-bottom:2px solid #e8ecf0;
}
.chart-container{flex:1;position:relative;min-height:300px;}

/* ── Right Side (Lesson Sidebar) ── */
.lesson-sidebar{
  background:#f7f9fb;border:1px solid #dde3ea;border-radius:12px;padding:1.5rem;
  overflow-y:auto;box-shadow:0 2px 8px rgba(0,0,0,.04);
}
.lesson-obj{
  background:#e8f4fd;color:#1565c0;padding:.85rem;border-radius:8px;
  font-size:.85rem;margin-bottom:1rem;border-left:4px solid #2980b9;line-height:1.4;
}
.lesson-obj strong{display:block;font-size:.7rem;text-transform:uppercase;letter-spacing:.05em;color:#2980b9;margin-bottom:.2rem;}
.q-box{
  background:#fffbf0;border:1px solid #ffeaa7;padding:.85rem;border-radius:8px;
  font-size:.85rem;color:#b8730a;margin-top:1rem;line-height:1.4;
}
.q-box strong{display:block;font-size:.7rem;text-transform:uppercase;letter-spacing:.05em;color:#d35400;margin-bottom:.2rem;}

/* ── Specific UI Elements ── */
.ts-legend{display:flex;gap:1rem;margin-top:1rem;flex-wrap:wrap;justify-content:center;}
.ts-leg-item{display:flex;align-items:center;gap:.35rem;font-size:.75rem;color:#6b7c93;font-weight:700}
.ts-leg-dot{width:12px;height:12px;border-radius:50%}

.stat-row{display:flex;justify-content:space-between;align-items:center;padding:.7rem .9rem;background:white;border-radius:8px;margin:.4rem 0;border:1px solid #eef1f5;}
.stat-row .label{font-size:.75rem;font-weight:700;color:#6b7c93;text-transform:uppercase;letter-spacing:.04em}
.stat-row .value{font-size:1.1rem;font-weight:900;color:#1a2332}
.stat-val-sm{font-size:.85rem;color:#6b7c93;font-weight:600;}

/* LOO Table */
.loo-table{width:100%;border-collapse:collapse;font-size:.8rem;}
.loo-table th{font-size:.65rem;font-weight:800;text-transform:uppercase;letter-spacing:.05em;color:#8a99ab;padding:.5rem;text-align:left;background:#f0f4f8;}
.loo-table td{padding:.5rem;border-bottom:1px solid #eef1f5;}

/* Discovery Log */
.disc-item{background:white;border:1px solid #e0e7ef;border-left:4px solid #27ae60;border-radius:8px;padding:.8rem;margin-bottom:.6rem;font-size:.85rem;}
.disc-item.warm{border-left-color:#e74c3c}
.disc-item.cold{border-left-color:#2980b9}
.disc-zone{font-size:.7rem;font-weight:800;text-transform:uppercase;color:#8a99ab;margin-bottom:.2rem;}
.disc-ngss{display:inline-block;padding:.2rem .5rem;background:#e8f4fd;color:#1565c0;border-radius:5px;font-size:.7rem;font-weight:800;margin-top:.4rem;}
</style>
</head>
<body>
...
<div class="dash-header">
  <div>
    <div class="dash-logo">ArgoBuddy <span>Command</span></div>
    <div style="font-size:.72rem;color:rgba(255,255,255,.45);margin-top:.15rem">
      Real-time telemetry · Pacific Ocean, Nov 2015 – Jan 2017
    </div>
  </div>
  <div class="dash-meta">
    <div><span class="live-dot"></span>Live Student Sync: <strong id="live-zone-status" style="color:white">Waiting...</strong></div>
    <a href="/" target="_blank" class="btn-back">← Student View</a>
  </div>
</div>

<div class="tab-nav">
  <button class="tab-btn active" onclick="openTab('tab-ts', this)">🌊 1. Water Identity</button>
  <button class="tab-btn" onclick="openTab('tab-layers', this)">📊 2. Ocean Layers</button>
  <button class="tab-btn" onclick="openTab('tab-stats', this)">📈 3. Statistical Safety</button>
  <button class="tab-btn" onclick="openTab('tab-log', this)">🚨 4. Discovery Log</button>
</div>

<div id="tab-ts" class="tab-pane active">
  <div class="module-grid">
    <div class="chart-box">
      <div class="panel-title">T–S Relationship Diagram (Cluster Analysis)</div>
      <div class="chart-container"><canvas id="ts-chart"></canvas></div>
      <div class="ts-legend">
        ...
        <div class="ts-leg-item"><div class="ts-leg-dot" style="background:#e74c3c;border:2px solid white"></div>Student's Float (Live)</div>
      </div>
    </div>
    <div class="lesson-sidebar">
      <div class="lesson-obj">
        <strong>Target Standard: NGSS MS-ESS3-5</strong>
        Analyze how specific water masses have unique "fingerprints" based on the relationship between temperature and salinity.
      </div>
      <div class="panel-title">Live Student Evidence</div>
      <div class="stat-row"><div class="label">Temperature</div><div class="value" id="ev-temp1">—</div></div>
      <div class="stat-row"><div class="label">Salinity</div><div class="value" id="ev-sal1">—</div></div>
      
      <div class="q-box">
        <strong>Discussion Prompt</strong>
        Ask the class: "If the red star moves outside the historical clusters, what does that mean for the ocean ecosystem?"
      </div>
    </div>
  </div>
</div>
...
<div id="tab-layers" class="tab-pane">
  <div class="module-grid">
    <div class="chart-box">
      <div class="panel-title">Vertical Profile (Salinity vs Depth) & Thermocline</div>
      <div class="chart-container"><canvas id="sal-prof-chart"></canvas></div>
    </div>
    <div class="lesson-sidebar">
      <div class="lesson-obj">
        <strong>Target Standard: NGSS MS-ESS2-6</strong>
        Visualize ocean stratification. Understand why the ocean has layers and how density/salinity acts as a barrier.
      </div>
      <div class="panel-title">Live Student Evidence</div>
      <div class="stat-row"><div class="label">Current Depth</div><div class="value" id="ev-depth2">—</div></div>
      <div class="stat-row"><div class="label">Current Zone</div><div class="value" id="ev-zone2">—</div></div>
      
      <div class="q-box">
        <strong>Discussion Prompt</strong>
        Ask the class: "Why does the blue line curve so sharply between 200m and 1000m? What acts as a barrier here?"
      </div>
    </div>
  </div>
</div>
...
<div id="tab-stats" class="tab-pane">
  <div class="module-grid">
    <div class="chart-box" style="overflow-y:auto">
      <div class="panel-title">Leave-One-Out (LOO) Baseline Analysis</div>
      <p style="font-size:.8rem;color:#6b7c93;margin-bottom:1rem;">This table represents the actual p-value calculation engine running behind the student game. If p &lt; 0.05, the anomaly triggers a health drain.</p>
      <table class="loo-table">
        <thead>
          <tr><th>Zone</th><th>Cycle</th><th>Historical Base (°C)</th><th>Anomaly (°C)</th><th>p-value</th></tr>
        </thead>
    ...
    <div class="lesson-sidebar">
      <div class="lesson-obj">
        <strong>Data Literacy Integration</strong>
        Teach students that the game isn't random. "Losing health" is mathematically tied to breaking the $1.5\sigma$ boundary of historical telemetry.
      </div>
      <div class="panel-title">Live Student Evidence</div>
      <div class="stat-row"><div class="label">Live p-value</div><div class="value" id="ev-pval3">—</div></div>
      <div class="stat-row"><div class="label">Status</div><div class="value stat-val-sm" id="ev-sig3">—</div></div>
      
      <div class="q-box">
        <strong>Discussion Prompt</strong>
        Ask the class: "Why do we use standard deviation instead of just guessing what a 'bad' temperature is?"
      </div>
    </div>
  </div>
</div>
...
<div id="tab-log" class="tab-pane">
  <div class="module-grid">
    <div class="chart-box" style="overflow-y:auto">
      <div class="panel-title">Live Anomaly Tracking (NGSS Events)</div>
      <div id="disc-list">
        <div style="text-align:center;padding:3rem;color:#8a99ab;">
          <div style="font-size:3rem;margin-bottom:1rem;">📡</div>
          Awaiting student anomalies...<br>Have the student adjust the sliders into the red zone to log a climate event.
        </div>
      </div>
    </div>
    <div class="lesson-sidebar">
      <div class="lesson-obj">
        <strong>Target Standard: NGSS MS-ESS3-5</strong>
        Connect micro-interactions (a single robot getting too hot) to macro-climate events (Coral Bleaching, Ocean Dead Zones).
      </div>
      <div class="panel-title">Float Vitals</div>
      <div class="stat-row"><div class="label">Float Health</div><div class="value" id="ev-health4" style="color:#27ae60">100%</div></div>
      
      <div class="q-box">
        <strong>Discussion Prompt</strong>
        Review the log with the class: "Which zone experienced the most severe anomaly today? What was the real-world consequence?"
      </div>
    </div>
  </div>
</div>
...
// ── Build T-S Chart (Tab 1) ───────────────────────────────────────────────────
const ctxTS = document.getElementById('ts-chart').getContext('2d');
const tsChart = new Chart(ctxTS, {
  type:'scatter',
  data:{datasets:[
    {label:'Sunlight',data:TS_DATA.sunlight||[],backgroundColor:'rgba(255,159,28,.35)',pointRadius:2.5,borderWidth:0},
    {label:'Twilight',data:TS_DATA.twilight||[],backgroundColor:'rgba(199,125,255,.35)',pointRadius:2.5,borderWidth:0},
    {label:'Midnight',data:TS_DATA.midnight||[],backgroundColor:'rgba(76,201,240,.35)',pointRadius:2.5,borderWidth:0},
    {label:'Student ★',data:[{x:33.36,y:12.2}],backgroundColor:'#e74c3c',pointRadius:14,pointHoverRadius:16,pointStyle:'star',borderColor:'white',borderWidth:1.5},
  ]},
  options:{
    responsive:true,maintainAspectRatio:false,animation:{duration:200},
    plugins:{legend:{display:false}},
    scales:{
      x:{title:{display:true,text:'Salinity (PSU)',color:'#6b7c93',font:{weight:'bold'}}, grid:{color:'rgba(0,0,0,.05)'}},
      y:{title:{display:true,text:'Temperature (°C)',color:'#6b7c93',font:{weight:'bold'}}, grid:{color:'rgba(0,0,0,.05)'}}
    }
  }
});

// ── Build Vertical Profile Chart (Tab 2) ──────────────────────────────────────
const ctxProf = document.getElementById('sal-prof-chart').getContext('2d');
if(SAL_PROF && SAL_PROF.length > 0) {
  new Chart(ctxProf, {
    type:'line',
    data:{labels:SAL_PROF.map(p=>p.d),datasets:[
      {label:'Mean Salinity',data:SAL_PROF.map(p=>p.sm),fill:false,borderColor:'#00d4ff',borderWidth:2.5,pointRadius:0,tension:.3},
      {label:'+1σ',data:SAL_PROF.map(p=>+(p.sm+p.ss).toFixed(3)),fill:'+1',backgroundColor:'rgba(0,212,255,.1)',borderColor:'rgba(0,212,255,.22)',borderDash:[3,3],borderWidth:1.5,pointRadius:0,tension:.3},
      {label:'-1σ',data:SAL_PROF.map(p=>+(p.sm-p.ss).toFixed(3)),fill:false,borderColor:'rgba(0,212,255,.22)',borderDash:[3,3],borderWidth:1.5,pointRadius:0,tension:.3}
    ]},
    options:{
      ...
      scales:{
        x:{title:{display:true,text:'Salinity (PSU)',color:'#6b7c93',font:{weight:'bold'}},grid:{color:'rgba(0,0,0,.05)'}},
        y:{title:{display:true,text:'Depth (dbar)',color:'#6b7c93',font:{weight:'bold'}},grid:{color:'rgba(0,0,0,.05)'}, reverse: true}
      }
    }
  });
}

// ── Build LOO Table (Tab 3) ───────────────────────────────────────────────────
(function(){
  const zColors={sunlight:'#b8730a',twilight:'#7b3fa3',midnight:'#0d7fa3'};
  const tbody=document.getElementById('loo-tbody');
  LOO_DATA.forEach(r=>{
    const tr=document.createElement('tr');
    const aSign=r.anomaly>=0?'+':'';
    const sigStyle=r.significant?'color:#e74c3c;font-weight:bold;':'color:#6b7c93;';
    tr.innerHTML=
      `<td><strong style="color:${zColors[r.zone]}">${r.zone.toUpperCase()}</strong></td>`+
      `<td>${r.cycle}</td>`+
      `<td>${r.loo_baseline.toFixed(3)}</td>`+
      `<td>${aSign}${r.anomaly.toFixed(3)}</td>`+
      `<td style="${sigStyle}">${r.p_value.toFixed(4)}</td>`;
...
function updateDashboard(d){
  ...
  document.getElementById('live-zone-status').textContent = `${d.zone.toUpperCase()} ZONE`;
  
  // Tab 1 Updates
  document.getElementById('ev-temp1').textContent = `${d.temperature}°C`;
  document.getElementById('ev-sal1').textContent = `${d.salinity} PSU`;
  tsChart.data.datasets[3].data=[{x:d.salinity, y:d.temperature}];
  tsChart.update('none');

  // Tab 2 Updates
  document.getElementById('ev-depth2').textContent = `${d.depth}m`;
  document.getElementById('ev-zone2').textContent = d.zone.toUpperCase();

  // Tab 3 Updates
  document.getElementById('ev-pval3').textContent = d.p_value.toFixed(4);
  document.getElementById('ev-sig3').textContent = d.significant ? '🚨 STATISTICAL ANOMALY' : '✓ Normal Variance';
  document.getElementById('ev-sig3').style.color = d.significant ? '#e74c3c' : '#27ae60';

  // Tab 4 Updates
  document.getElementById('ev-health4').textContent = `${d.health}%`;
  document.getElementById('ev-health4').style.color = d.health < 50 ? '#e74c3c' : '#27ae60';
...
function renderDiscoveries(list){
  ...
  el.innerHTML=[...list].reverse().map(d=>{
    ...
    return `<div class="disc-item ${cls}">
      <div style="float:right;font-size:.7rem;color:#8a99ab">${d.time}</div>
      <div class="disc-zone">${d.zone} · ${d.mood}</div>
      <div style="font-weight:bold;margin-bottom:.3rem">T=${d.temp}°C | S=${d.sal} PSU | p=${d.p_value.toFixed(4)}</div>
      ${d.ngss_code?`<div class="disc-ngss">${d.ngss_code}: ${d.ngss_desc}</div>`:''}
    </div>`;
  }).join('');
}
